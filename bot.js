...
const mongoURI = `mongodb+srv://${mongo_user}:${mongo_pass}@cluster0.fo2dm.mongodb.net/botDB`;
...
bot.on("/start", (msg) => {
  ...
  return bot.sendMessage(
    msg.from.id,
    `Hi ${msg.from.first_name}! This is ramu kaka's shop bot. You can follow the command menu on the left side of your textbox to interact with the bot. Hope you have a great time shopping!`
  );
});
...
bot.on("/display_inventory", (msg) => {
  inventoryModel.find({}).then((inventory) => {
    ...
    inventory.forEach((item) => {
      inventoryString += `${item.Name} - Rs${item.Price} - ${item.Stock}Kg remain - (${item.Type})\n`;
    });
...
bot.on("/order_cart", (msg) => {
  // Send the user @kumararyanmfp a confirmation regarding the order, if they confirm, then add the cart to the orders and empty the cart, also notify the user that the order has been placed
  let order = ``;
  UserModel.find({ UserId: msg.from.id }).then((user) => {
    let cart = user[0].Cart;
    if (cart.length == 0) return bot.sendMessage(msg.from.id, "Cart is empty");
